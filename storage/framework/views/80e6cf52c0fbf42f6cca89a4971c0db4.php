...
<head>
    ...
    ?>
    <?php if($__seo && $__seo->seo_title): ?>
        <title><?php echo e($__seo->seo_title); ?></title>
    <?php else: ?>
        <title><?php echo $__env->yieldContent('title', 'High Contrast Detailing Center | Car Detailing Premium en Chicureo'); ?></title>
    <?php endif; ?>
    <link rel="icon" type="image/png" href="<?php echo e($profile && $profile->logo ? asset($profile->logo) : asset('assets/logos/main-logo.png')); ?>">
    
    <!-- Meta tags for SEO -->
    <?php if($__seo && $__seo->seo_description): ?>
        <meta name="description" content="<?php echo e($__seo->seo_description); ?>">
    <?php else: ?>
        <meta name="description" content="<?php echo $__env->yieldContent('meta_description', 'Centro de detailing automotriz premium en Chicureo, Chile. Pulido profesional, tratamiento cerámico, detailing interior y exterior.'); ?>">
    <?php endif; ?>
    <meta name="keywords" content="<?php echo $__env->yieldContent('meta_keywords', 'car detailing, detailing chicureo, pulido de autos, tratamiento cerámico, detailing premium, lavado premium, corrección de pintura, High Contrast Detailing'); ?>">
    ...
    <?php if($profile && !empty($profile->google_analytics_id)): ?>
        <!-- Global site tag (gtag.js) - Google Analytics -->
        <script async src="https://www.googletagmanager.com/gtag/js?id=<?php echo e($profile->google_analytics_id); ?>"></script>
        <script>
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', '<?php echo e($profile->google_analytics_id); ?>');
        </script>
    <?php endif; ?>

    <?php if($profile && !empty($profile->google_tag_manager_id)): ?>
        <!-- Google Tag Manager -->
        <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
        new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
        j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
        'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
        })(window,document,'script','dataLayer','<?php echo e($profile->google_tag_manager_id); ?>');</script>
    ...
    <?php if($profile && !empty($profile->header_scripts)): ?>
        <?php echo $profile->header_scripts; ?>
...
<body class="antialiased bg-gray-50 text-black dark:bg-[#0A0A0A] dark:text-[#F5F5F5] transition-colors duration-300" x-data="{ isMobileOpen: false }">
    <?php if($profile && !empty($profile->google_tag_manager_id)): ?>
        ...
        <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=<?php echo e($profile->google_tag_manager_id); ?>"
    ...
    <!-- Navbar -->
    <nav x-data="{ 
        isScrolled: <?php echo e($forceSolidNav ? 'true' : 'false'); ?>, 
        isServicesOpen: false 
    }" 
    @scroll.window="isScrolled = <?php echo e($forceSolidNav ? 'true' : 'window.pageYOffset > 50'); ?>"
        ...
        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 flex items-center justify-between">
            ...
            <a href="/" class="group flex items-center gap-3">
                <div class="relative h-10 w-10">
                    <img src="<?php echo e($profile->logo ? asset($profile->logo) : asset('assets/logos/main-logo.png')); ?>" alt="<?php echo e($profile->business_name ?? 'High Contrast'); ?>" class="object-contain w-full h-full">
                </div>
            ...
            <div class="hidden md:flex items-center gap-2">
                <a href="/" :class="isScrolled ? '<?php echo e(request()->is('/') ? 'text-brand font-bold' : 'text-black/70 dark:text-white/70 hover:text-brand dark:hover:text-brand'); ?>' : '<?php echo e(request()->is('/') ? 'text-brand font-bold' : 'text-white/70 hover:text-white'); ?>'" class="px-4 py-2 text-sm font-medium transition-all">Inicio</a>
                <a href="/nosotros" :class="isScrolled ? '<?php echo e(request()->is('nosotros') ? 'text-brand font-bold' : 'text-black/70 dark:text-white/70 hover:text-brand dark:hover:text-brand'); ?>' : '<?php echo e(request()->is('nosotros') ? 'text-brand font-bold' : 'text-white/70 hover:text-white'); ?>'" class="px-4 py-2 text-sm font-medium transition-all">Nosotros</a>
                
                <!-- Dropdown Services (Simple Menu) -->
                <div class="relative" @mouseenter="isServicesOpen = true" @mouseleave="isServicesOpen = false">
                    <button :class="isScrolled ? '<?php echo e($isServicesActive ? 'text-brand font-bold' : 'text-black/70 dark:text-white/70 hover:text-brand dark:hover:text-brand'); ?>' : '<?php echo e($isServicesActive ? 'text-brand font-bold' : 'text-white/70 hover:text-white'); ?>'" class="flex items-center gap-1 px-4 py-2 text-sm font-medium transition-all">
                        Servicios
                    ...
                    <div x-show="isServicesOpen" x-transition class="absolute left-1/2 -translate-x-1/2 top-full pt-3" style="width: 320px; display: none; z-index: 50;">
                        <div class="overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-white/95 dark:bg-[#1A1A1A]/95 shadow-xl backdrop-blur-xl" style="padding: 10px;">
                            ...
                            <div class="flex flex-col" style="gap: 6px;">
                                <?php if(isset($navCategoryOrder)): ?>
                                    <?php $__currentLoopData = $navCategoryOrder; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $catId): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php if(isset($catDetails[$catId])): ?>
                                            <?php $detail = $catDetails[$catId]; ?>
                                            <a href="<?php echo e($detail['url']); ?>" class="flex items-center rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-200 group" style="padding: 10px; gap: 14px;">
                                                <div class="rounded-lg transition-all duration-300 group-hover:scale-110 flex items-center justify-center shrink-0" style="width: 38px; height: 38px; background-color: <?php echo e($detail['bg_raw']); ?>; color: <?php echo e($detail['color_raw']); ?>">
                                                    <?php echo $detail['icon']; ?>

                                                </div>
                                                <div class="font-display font-bold text-sm tracking-tight text-black dark:text-white transition-colors duration-200" style="white-space: nowrap; --hover-color: <?php echo e($detail['color_raw']); ?>">
    ...
    <div x-show="isMobileOpen" x-transition class="md:hidden fixed inset-0 z-50 overflow-y-auto px-8 pt-24 mobile-nav-panel" style="display: none;">
        ...
        <div class="flex flex-col gap-4 pb-12">
            <a href="/" @click="isMobileOpen = false" class="px-5 py-4 font-display text-2xl font-bold mobile-nav-link <?php echo e(request()->is('/') ? 'active' : ''); ?>">Inicio</a>
            <a href="/nosotros" @click="isMobileOpen = false" class="px-5 py-4 font-display text-2xl font-bold mobile-nav-link <?php echo e(request()->is('nosotros') ? 'active' : ''); ?>">Nosotros</a>
            ...
            <div class="space-y-3 rounded-3xl p-5 mobile-services-box">
                ...
                <?php if(isset($navCategoryOrder)): ?>
                    <?php $__currentLoopData = $navCategoryOrder; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $catId): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if(isset($catDetails[$catId])): ?>
                            <?php $detail = $catDetails[$catId]; ?>
                            <a href="<?php echo e($detail['url']); ?>" @click="isMobileOpen = false" class="flex items-center font-bold mobile-service-link text-white transition-all duration-200" style="padding: 10px 0; gap: 16px;">
                                <span class="rounded-xl flex items-center justify-center shrink-0" style="width: 40px; height: 40px; background-color: <?php echo e($detail['bg_raw']); ?>; color: <?php echo e($detail['color_raw']); ?>">
                                    <?php echo $detail['icon']; ?>

                                </span>
                                <span class="text-base tracking-tight" style="white-space: nowrap;"><?php echo e($detail['label']); ?></span>
                            </a>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
            ...
            <?php if(auth()->check() && (auth()->user()->isAdmin() || auth()->user()->isStaff())): ?>
                <a href="<?php echo e(route('admin.dashboard')); ?>" @click="isMobileOpen = false" class="mt-4 rounded-full bg-white/10 border border-white/20 px-8 py-4 text-center text-lg font-semibold text-white shadow-lg flex items-center justify-center gap-2 hover:bg-white/20 transition-all">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
                ...
                <div class="lg:col-span-1">
                    <div class="flex items-center gap-3.5 mb-5">
                        ...
                        <div>
                            <p class="font-display font-extrabold text-white tracking-tight text-lg">
                                <?php echo e(strtoupper($profile->business_name ?? 'High Contrast')); ?>
                    ...
                    <a href="https://instagram.com/<?php echo e(ltrim($profile->instagram ?? 'highcontrastdc', '@')); ?>"
                       target="_blank"
                       ...
                       class="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/5 border border-white/15 text-white/80 hover:text-brand hover:bg-white/10 transition-all text-xs font-bold uppercase tracking-wider">
                        ...
                        <span><?php echo e('@' . ltrim($profile->instagram ?? 'highcontrastdc', '@')); ?></span>
                    </a>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-3.5">
                        <li class="flex items-start gap-3 text-white/60 text-sm font-light">
                            ...
                            <span><?php echo e(($profile->address_line1 ?? 'Chicureo') . ($profile->address_line2 ? ', ' . $profile->address_line2 : '') . ', ' . ($profile->city ?? 'Colina')); ?></span>
                        </li>
                        <li>
                            <a href="tel:<?php echo e(preg_replace('/\s+/', '', $profile->phone ?? '+56912345678')); ?>" class="flex items-center gap-3 text-white/60 hover:text-brand transition-colors text-sm font-light">
                                ...
                                <?php echo e($profile->phone ?? '+56 9 1234 5678'); ?>

                            </a>
                        </li>
                        <li>
                            <a href="mailto:<?php echo e($profile->email ?? 'info@highcontrastdetailing.cl'); ?>" class="flex items-center gap-3 text-white/60 hover:text-brand transition-colors text-sm font-light">
                                ...
                                <?php echo e($profile->email ?? 'info@highcontrastdetailing.cl'); ?>
            ...
            <div class="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
                <p class="text-white/40 text-xs font-light">
                    © <?php echo e(date('Y')); ?> <?php echo e($profile->business_name ?? 'High Contrast'); ?>. Todos los derechos reservados. Chicureo, Chile.
                </p>
    ...
    <div x-data="{ isTooltipOpen: false }" class="fixed bottom-6 right-6 z-50">
        <div x-show="isTooltipOpen"
            ...
            <div class="p-5 rounded-2xl bg-[#1A1A1A] border border-white/10 shadow-2xl shadow-black/50">
                ...
                <a href="https://wa.me/<?php echo e($whatsappNumber); ?>?text=<?php echo e($whatsappMsg); ?>"
                   target="_blank"
                   rel="noopener noreferrer"
    ...
    <?php if($profile && !empty($profile->footer_scripts)): ?>
        <?php echo $profile->footer_scripts; ?>
...
<?php /**PATH C:\laragon\www\detailing-laravel\resources\views/layouts/public.blade.php ENDPATH**/ ?>